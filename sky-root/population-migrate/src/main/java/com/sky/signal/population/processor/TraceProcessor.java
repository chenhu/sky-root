...
public class TraceProcessor implements Serializable {
    ...
    public DataFrame filterStayMsisdnTrace(final DataFrame traceDf) {
        JavaPairRDD<String, List<Row>> signalRdd = SignalProcessUtil.keyPairByMsisdn(traceDf,
                params);
        ...
        signalRdd = signalRdd.filter(new Function<Tuple2<String, List<Row>>, Boolean>() {
            ...
            public Boolean call(Tuple2<String, List<Row>> rows) throws Exception {
                ...
                if (signals.size() > 1) {
                    Timestamp beginTime = signals.get(0).getAs("begin_time");
                    ...
                    if (timeDiffSeconds >= DISTRICT_STAY_MINUTE * 60) {
                        return true;
                    } else {
                        return false;
                    }
        ...
        JavaRDD<Row> resultRdd = signalRdd.values().map(new Function<List<Row>, Row>() {
            ...
            public Row call(List<Row> rows) throws Exception {
                ...
                return new GenericRowWithSchema(new Object[]{row.getAs("date"), row.getAs
                        ("msisdn"), row.getAs("region"), row.getAs("city_code"), row.getAs
                        ("district"), beginTime, endTime, moveTime}, SignalSchemaProvider
                        .DISTRICT_SIGNAL);
            }
        });
     ...
     * @return
     */
    public DataFrame mergeCellSignal(DataFrame traceDf, final Broadcast<Map<String, Row>>
            provinceCell) {
        JavaRDD<Row> rdd = traceDf.javaRDD().map(new Function<Row, Row>() {
            ...
            public Row call(Row row) throws Exception {
                ...
                Integer date = Integer.valueOf(DateTime.parse(startTime, FORMATTER).toString
                        (FORMATTED));
                ...
                Integer city_code = 0;
                Timestamp begin_time = new Timestamp(DateTime.parse(startTime, FORMATTER)
                        .getMillis());
                ...
                Row track = RowFactory.create(date, msisdn, region, city_code, district, tac,
                        cell, base, lng, lat, begin_time, end_time);
                return track;
            }
        });
        ...
        DataFrame validDf = traceDf.filter(col("msisdn").notEqual("null").and(col("base")
                .notEqual("null")).and(col("lng").notEqual(0)).and(col("lat").notEqual(0)))
                .dropDuplicates(new String[]{"msisdn", "begin_time"});
        return validDf;
    }
