...
public class StayPointProcessor implements Serializable {
     ...
     **/
    private Tuple3<List<Row>, List<Row>, List<Row>> filterOD(List<Row> rows) {
        ...
        //筛选od轨迹
        for (Tuple3<String, String, Timestamp> tuple3 : odTraceMap.keySet()) {
            String beginBase = tuple3._1();
            String endBase = tuple3._2();
            LinkedList<Row> trace = (LinkedList<Row>) odTraceMap.get(tuple3);
            for (Row row: odResult) {
                if(beginBase.equals(row.getAs("leave_base")) && endBase.equals(row.getAs("arrive_base"))) {
                    if (trace.size() > 1) {
    ...
    public void process(String validSignalFile) {
        ...
        JavaRDD<Tuple4<List<Row>,List<Row>,Row,List<Row>>> analyzedRdd = transSignalPairRdd.values().map(new Function<List<Row>, Tuple4<List<Row>, List<Row>, Row, List<Row>>>() {
            ...
            public Tuple4<List<Row>, List<Row>, Row, List<Row>> call(List<Row> rows) throws Exception {
                ...
                Tuple3<List<Row>, List<Row>, List<Row>> result = filterOD(rows);
                // 有效的ODTrace，包括中间的位移点
