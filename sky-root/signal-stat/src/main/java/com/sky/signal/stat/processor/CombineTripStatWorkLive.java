...
public class CombineTripStatWorkLive implements Serializable {
    ...
    public DataFrame process(DataFrame statTripDf, DataFrame workLiveStat, Integer batchId) {
        ...
        FileUtil.saveFile(odTripStat, FileUtil.FileType.CSV, params.getODTripStatSavePath(batchId.toString()));

        return odTripStat;
    }

    public DataFrame read() {
        return FileUtil.readFile(FileUtil.FileType.CSV, ODSchemaProvider.OD_TRIP_STAT_SCHEMA,params.getODTripStatSavePath("*")).repartition(params.getPartitions());
    }
}
